...
public class ProductivityFragment extends Fragment {
    ...
    LinkedList<Number> productivitySeries = new LinkedList<>();
    ...
    private void loadData(){
        ...
        for(int i = 0; i < NUM_DISPLAYED_MONTHS; ++i){
            ...
            int totalTasks = getTotalTasks(month);
            if(totalTasks != 0) productivity = ((float)getCompletedTasks(month) / (float)totalTasks) * 100;
            productivitySeries.addFirst(productivity);
            monthList.addFirst(month-1);

            month = (month - 1) % 12;
        }

        productivitySeries.addFirst(null);
    }
    ...
    private void updatePlot() {
        ...
        series1 = new SimpleXYSeries(productivitySeries, SimpleXYSeries.ArrayFormat.Y_VALS_ONLY, "");
        ...
        final BarRenderer.BarGroupWidthMode barGroupWidthMode = (BarRenderer.BarGroupWidthMode) BarRenderer.BarGroupWidthMode.FIXED_WIDTH;
        renderer.setBarGroupWidth(barGroupWidthMode, 100);
